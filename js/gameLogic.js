...
function startGame() {
  ...
  stocks.forEach(s => {
    s.history = [s.price]; // reset history
    players.forEach((_, i) => {
      s.owned[i] = 0;
      s.totalSpent[i] = 0;
    });
...
function sell(i, amount = 1) {
  ...
  let sellAmount = Math.min(amount, s.owned[currentPlayer]);
  s.owned[currentPlayer] -= sellAmount;
  let avg = s.totalSpent[currentPlayer] / (s.owned[currentPlayer] + sellAmount);
  s.totalSpent[currentPlayer] -= avg * sellAmount;
  players[currentPlayer].money += s.price * sellAmount;
...
function applyDividends() {
  players.forEach((p, pi)=>{
    stocks.forEach(s=>{
      ...
      let rate = owned>=2000 ? 0.2 : owned>=1000 ? 0.1 : owned>=500 ? 0.075 : owned>=100 ? 0.05 : owned>=50 ? 0.025 : owned>10 ? 0.005 : 0;
      p.money += value*rate;
    });
  });
}
